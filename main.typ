...
+ Методы кластеризации, которые позволяют объединять схожие по характеристикам участки временного ряда, выявляя повторяющиеся паттерны в трафике (утренние и вечерние часы пик, ночное снижение интенсивности и т.д.) @clustering
...
Алгоритм k-means является одним из наиболее используемых методов кластеризации благодаря простоте и эффективности @k-means. Он минимизирует внутрикластерную дисперсию, итеративно распределяя объекты по k кластерам. Преимущества: быстрая сходимость, масштабируемость и лёгкость реализации. Ограничения: предполагает сферическую форму кластеров схожего размера, чувствителен к выбросам. На практике может не обнаружить заданное число кластеров, особенно при нечётко разделённых данных.

// DBSCAN

DBSCAN -- метод кластеризации, основанный на анализе плотности данных @DBSCAN. Объединяет объекты в кластеры, если они образуют плотную область в пространстве признаков. Требует два параметра: $epsilon$ (радиус окрестности) и $M i n P t s$ (минимальное количество точек). Преимущества: выявляет кластеры произвольной формы, устойчив к выбросам, автоматически определяет количество кластеров. Недостатки: не предоставляет механизма для получения строго заданного числа кластеров. Плохо работает при равномерном или чрезмерно плотном распределении данных, что делает его неприменимым в задачах с высокой плотностью и временной структурой.

// Gaussian Mixture Model, GMM

GMM предполагает, что данные сгенерированы из комбинации нескольких нормальных распределений @gmm. Использует EM-алгоритм для оценки параметров и формирует вероятностное разбиение данных. Позволяет моделировать эллипсоидные формы кластеров и хорошо работает с перекрывающимися группами. Ограничения: требует нормального распределения данных. В задачах с дискретными, асимметричными данными, близкими к распределению Пуассона, использование GMM некорректно.

// Poisson Mixture Model, PMM

PMM -- вероятностная модель для данных, подчиняющихся распределению Пуассона @pmm. Хорошо подходит для счётных величин и частотных признаков. Использует EM-алгоритм и обеспечивает высокую интерпретируемость параметров в терминах интенсивности пуассоновских процессов. Ограничения: предполагает именно пуассоновское распределение признаков, чувствителен к мультиколлинеарности и выбросам. Несмотря на это, для задач со счётными значениями представляет более адекватный подход, чем гауссовская альтернатива.

// Агломеративная кластеризация

Иерархический метод, строящий кластеры снизу вверх: каждый объект изначально -- отдельный кластер, затем кластеры последовательно объединяются. Метод Уорда, основанный на минимизации приращения внутрикластерной дисперсии, обеспечивает сбалансированное разбиение и подходит для структур с компактностью и раздельностью кластеров @agglomerative.

// Алгоритм FINCH

FINCH -- агломеративный алгоритм, основанный на связях первого ближайшего соседа @finch. Не требует настройки гиперпараметров, но позволяет контролировать число кластеров через остановку процесса на нужной итерации. Недостатки: не оптимизирует расстояние между кластерами, что может приводить к слабо различимым группам. Чувствителен к локальным структурам и выбросам.

// Алгоритм TW-FINCH

TW-FINCH -- модификация FINCH для временных данных с введением временных весов @tw-finch. Сохраняет достоинства базовой версии: не требует задания параметров, но позволяет получить заданное число кластеров. Недостатки: как и FINCH, не максимизирует расстояние между кластерами. Качество зависит от корректного задания временных весов.
...
+ реализацию механизмов фильтрации, агрегации и заполнения пропусков;
...
+ визуализацию карт времени и суточной интенсивности;

+ реализацию серверной части с REST API для взаимодействия с пользовательским интерфейсом;
...
#figure(
    image("схемы/структурная_схема_системы.png", width: 60%),     caption: "Схема взаимодействия модулей в системе"
...
#figure(image("assets/1.png", width: 76%),
    caption: "Макет первого шага" 
) <step-1>

#figure(image("assets/2.png", width: 76%),
    caption: "Макет второго шага" 
) <step-2>

#figure(image("assets/3.png", width: 76%),
    caption: "Макет третьего шага" 
) <step-3>

#figure(image("assets/4.png", width: 76%),
...
#figure(image("assets/client-server.png", width: 55%),
...
Модуль анализирования, схема которого представлена на рисунке @analizis_module_scheme, представляет собой центральный компонент системы, отвечающий за выполнение анализа временных рядов. 

#figure(image("схемы/схема_модуля_анализирования_для_отчета.png", height: 55%),
...
где $mu_s$ -- центроид сегмента $s$;

#h(20pt) $I_s$ -- множество индексов элементов, принадлежащих сегменту $s$;

#h(20pt) $x_i in RR^d$ -- вектор размерности $d$ для элемента $i$.
...
#h(20pt) $S$ -- общее число сегментов;

#h(20pt) $I_s$ -- множество индексов элементов в сегменте $s$;

#h(20pt) $x_i in RR^d$ -- вектор размерности $d$ для элемента $i$;

#h(20pt) $mu_s$ -- центроид сегмента $s$, вычисленный по формуле @mu.
...
#h(20pt) $mu_s$ и $mu_(s+1)$ -- центроиды соседних сегментов $s$ и $s+1$, вычисленные по формуле @mu.
...
где $S S R$ -- доля сегментов, не удовлетворяющих минимальной длине;

#h(20pt) $S$ -- общее число сегментов;

#h(20pt) $L_min$ -- минимальная допустимая длина сегмента.
...
где $S E$ -- отклонение числа сегментов;

#h(20pt) $S$ -- фактическое общее число сегментов;

#h(20pt) $S^*$ -- требуемое общее число сегментов.
...
где $V_norm$ -- нормализованная внутрисегментная вариация;

#h(20pt) $V$ -- внутрисегментная вариация, вычисленная по формуле @V;

#h(20pt) $x_i$, $x_j$ $in RR^d$ -- вектора размерности $d$ для элементов $i$, $j$.
...
где $D_norm$ -- нормализованная внутрисегментная вариация;

#h(20pt) $D$ -- внутрисегментная вариация, вычисленная по формуле @D;
...
#figure(image("схемы/схема_генетического_алгоритма.png", height: 75%),
    caption: "Блок-схема генетического алгоритма" 
) <genetic-algorithm>
...
где $D_(n o r m)$ -- нормализованная сумма евклидовых расстояний между средними векторами сегментов;

#h(20pt) $P_(s i z e)$ -- штраф за неравномерность размеров сегментов;

#h(20pt) $P_(r a n g e)$ -- штраф за неоднородность диапазонов значений в сегментах;

#h(20pt) $w_1$, $w_2$, $w_3$ -- весовые коэффициенты важности критериев.
...
#h(20pt) $V_i$ – объём функций ПО;

#h(20pt) $n$ – общее число функций.
...
$
    ...
    text(",", font: "Times New Roman")
$ <V_y>
где	$V_(y i)$ – уточнённый объём отдельной функции в строках исходного кода.
...
где $Т_(с т .1р)$ – базовая ставка 270 рублей, утверждённая согласно ЕТС РБ на дату написания дипломного проекта);

#h(20pt) $Т_к$ – тарифный коэффициент согласно разряду исполнителя;

#h(20pt) $Ф_(э ф ф.р.в.)$ – среднее количество рабочих дней;

#h(20pt) $Ф_(р в)$ – фонд рабочего времени исполнителя (продолжительность разработки программного модуля, дни);

#h(20pt) $К_(п р)$ – коэффициент премии, $К_(п р) = 1,5$.
...
где	$Ц_(м в i)$  – цена одного машинного часа (50 бел. руб.);

#h(20pt) $V_0$ – уточнённый общий объём машинного кода. Согласно расчётам пункта @V_y данное значение равно 1905 строк;

#h(20pt) $Н_(м в)$  – норматив расхода машинного времени на отладку 100 строк кода, машино-часов. Принимается в размере 0,6. 
...
где	$Н_(н р)$ – норматив накладных расходов, в данном дипломном проекте норматив накладных расходов равен 40%.
...
где	$С З$ – сумма вышеперечисленных расходов по статьям на разработку программного продукта;

#h(20pt) $Н_о$ – установленный норматив затрат на освоение. Для данного дипломного проекта принимается равной 5%.
...
#h(20pt) $Н_(с о)$ – установленный норматив затрат на сопровождение программного продукта. Для данного дипломного проекта принимается равной 5%. 
...
где	$С П$ – полная себестоимость программного модуля, бел. руб;

#h(20pt) $R$ – уровень рентабельности программного модуля. В данном дипломном проекте уровень рентабельности равен 30%.
...
где	$С П$ – полная себестоимость программного модуля управления ESP32 на базе JavaScript, бел. руб;

#h(20pt) $П$ – плановая прибыль от реализации программного модуля, бел. руб. 
...
#h(20pt) $Н Д С_(п п)$ – налог на добавленную стоимость.
...
где $Ц_п$ – прогнозируемая цена, бел. руб.; 

#h(20pt) $Н Д С$ -- налог на добавленную стоимость. В настоящее время он составляет 20%.
...
#h(20pt) $Н_п$ – ставка налога на прибыль. В настоящее время он равен 20%.
...
#numless[= Приложение А. Текст программы]
